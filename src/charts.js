...
const shortFor = {};
DATA.participants.forEach(p => { shortFor[p.name] = p.short; });
...
const vocabSorted = top15Full
  .map(name => ({ name: shortFor[name], ...DATA.vocab_richness[name] }))
...
const respSorted = top15Full
  .map(name => ({ name: shortFor[name], ...DATA.response_times[name] }))
...
const startersSorted = top15Full
  .map(name => ({ name: shortFor[name], count: DATA.conversation_starters[name] || 0 }))
...
const nightData = top15Full
  .map(name => ({ name: shortFor[name], pct: DATA.fun_stats.night_pct[name] || 0 }))
...
const streakData = top15Full
  .map(name => ({ name: shortFor[name], streak: DATA.fun_stats.consecutive_day_streaks[name] || 0 }))
...
yearlyPeopleArr.slice(0, 15).forEach((person, i) => {
  yearlyDatasets.push({
    label: shortFor[person] || person.split(' ')[0],
    data: years.map(y => {
